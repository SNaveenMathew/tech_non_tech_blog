...
The environment is represented by a discretized state space, inspired by the previous repo. The reward function is defined as shown below:
...
During training the mean reward improved from about −17,600 to a best of around −5,500 for the first ~250 episodes. Then, between episodes 250 and 500, the reward dropped to −24,000 to −38,500, and the DQN's Huber loss climbed roughly 100x. The problem wasn't the reward function's shape — it was scale: nothing was clipping or rescaling the accumulated reward before it hit the Bellman target.
...
This project's visualizer — filling FlightPathAnalysis's fourth stage — was inspired by [flightradar24.com](https://www.flightradar24.com/).
...
Every fix in this project — the reward scale, the Leaflet event-ordering bug — got reverted, re-tested to confirm it reproduces the reported symptom, and restored. A fix that only "looks right" in a code diff isn't verified.
